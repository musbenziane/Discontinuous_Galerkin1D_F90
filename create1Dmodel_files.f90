...
program create1Dmodel_files
    ...
    character(len=40)                        :: modnamevp, modnamerho
    ...
    allocate(v1D(nel))
    allocate(rho1D(nel))
    modnamerho = "testing_rho"
    modnamevp  = "testing_vp"
    ...
    select case (modtype)
        ...
        v1D = 2000
        rho1D = 1800
        open(9,file=modnamevp,access='direct',recl=nel*8)
        write(9,rec=1) v1D
        close(9)

        open(8,file=modnamerho,access='direct',recl=nel*8)
        write(8,rec=1) rho1D
        close(8)
        ...
        open(10,file=modnamevp,access='direct',recl=nel*8)
        write(10,rec=1) v1D
        close(10)

        open(11,file=modnamerho,access='direct',recl=nel*8)
        ...
        open(12,file=modnamevp,access='direct',recl=nel*8)
        write(12,rec=1) v1D
        close(12)

        open(13,file=modnamerho,access='direct',recl=nel*8)
